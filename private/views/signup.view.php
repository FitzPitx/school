<?php $this->view('includes/header')?>
	...
	<div class="container-fluid">
		...
		<div class="p-4 mx-auto mr-4 shadow rounded" style="margin-top: 50px;width:100%;max-width: 340px;">
			...
			<input class="my-2 form-control" value="<?=get_var('email')?>" type="email" name="email" placeholder="Correo electrónico" >

			<select class="my-2 form-control" name="gender">
				<option <?=get_select('gender','')?> value="">--Selecciona el genero--</option>
				<option <?=get_select('gender','male')?> value="male">Masculino</option>
				<option <?=get_select('gender','female')?> value="female">Femenino</option>
			</select>
			...
			<?php else:?>
				<select class="my-2 form-control" name="rank">
					<option <?=get_select('rank','')?> value="">--Selecciona un rol--</option>
					<option <?=get_select('rank','student')?> value="student">Estudiante</option>
					<option <?=get_select('rank','reception')?> value="reception">Director de curso</option>
			...
			<input class="my-2 form-control" value="<?=get_var('password2')?>" type="text" name="password2" placeholder="Reescribir contraseña">
			<br>
			<button class="btn btn-primary float-end">Añadir usuarior</button>
